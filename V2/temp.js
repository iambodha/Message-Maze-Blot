...
function importPattern(character, callback) {
  const xhr = new XMLHttpRequest();
  xhr.open('GET', `https://raw.githubusercontent.com/iambodha/Message-Maze-Blot/main/Updated-MessageMaze/assets/patterns/${character}.json`, true);
  xhr.onload = function() {
    if (xhr.status === 200) {
...
function generateMazeWithTextSolution(inputText, callback) {
  const inputTextArr = formatInput(inputText);

  function processCharMazes(charMazesLists) {
  ...
  function importPatterns(charList, index, charMazes, callback) {
    if (index >= charList.length) {
      callback(null, charMazes);
      return;
    }

    importPattern(charList[index], (error, maze) => {
      if (error) {
        callback(error);
        return;
      }
      ...
      charMazes.push(maze);
      importPatterns(charList, index + 1, charMazes, callback);
    });
  }

  function processLines(lines, index, charMazesLists, callback) {
    if (index >= lines.length) {
      callback(null, charMazesLists);
      return;
    }

    importPatterns(lines[index], 0, [], (error, charMazes) => {
      if (error) {
        callback(error);
        return;
      }
      ...
      processLines(lines, index + 1, charMazesLists, callback);
    });
  }

  processLines(inputTextArr, 0, [], (error, charMazesLists) => {
    if (error) {
      callback(error);
      return;
    }

    processCharMazes(charMazesLists);
  });
}
...
function generateMazeListFromString(inputString, callback) {
  // Sanitize input string
  inputString = inputString.replace(/[^a-zA-Z]/g, ' ').replace(/\s+/g, ' ').trim();

  if (inputString.length === 0) {
    callback(new Error('Input string is empty!'));
    return;
  }

  if (inputString.length > 30) {
    callback(new Error('Input string is too long! Please keep it under 30 characters.'));
    return;
  }

  // Generate the maze
  generateMazeWithTextSolution(inputString, (error, maze) => {
    if (error) {
      callback(error);
      return;
    }
    ...
    callback(null, mazeList);
  });
}

function drawMazeFromString(hiddenMessage) {
  generateMazeListFromString(hiddenMessage, (error, mazeData) => {
    if (error) {
